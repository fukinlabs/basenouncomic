...
contract WebPArtNFT is Initializable, ERC721URIStorageUpgradeable, OwnableUpgradeable {
    ...
    function initialize(address initialOwner) public initializer {
        __ERC721_init("WebP Art NFT", "WEBP");
    ...
    function mintForFid(
        ...
        string memory json = string(
            abi.encodePacked(
                '{"name":"WebP Art #', _uint2str(fid),
                '", "description":"WebP generated NFT bound to Farcaster FID ', _uint2str(fid),
    ...
    /**
     * Check if string is IPFS hash (starts with "ipfs://")
     */
    ...
    /**
     * Check if base64 string is JPEG
     * JPEG base64 typically starts with "/9j/" (0x2F 0x39 0x6A 0x2F)
     */
    ...
    /**
     * Check if base64 string is HTML
     */
    function _isHtmlBase64(string memory base64String) internal pure returns (bool) {
        bytes memory base64Bytes = bytes(base64String);
        
        if (base64Bytes.length >= 7) {
            // Check for "PHRtbCB" (HTML tag in base64)
            if (
                base64Bytes[0] == 0x50 && // 'P'
                base64Bytes[1] == 0x48 && // 'H'
                base64Bytes[2] == 0x52 && // 'R'
                base64Bytes[3] == 0x74 && // 't'
                base64Bytes[4] == 0x62 && // 'b'
                base64Bytes[5] == 0x57 && // 'W'
                base64Bytes[6] == 0x77    // 'w'
            ) {
                return true;
            }
        }
        ...
        if (base64Bytes.length >= 20) {
            // Check for "PCFET0NUWVBFIGh0bWw" (DOCTYPE html in base64)
